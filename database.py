...
def init_db():
    ...
    with db_transaction() as conn:
        ...
        # Migrate old sequential barcodes (CNX-1, CNX-0001, etc.) to scrambled format.
        ...
        for idx, row in enumerate(old_barcodes, 1):
            ...
            try:
                old_num = _base36_to_int(suffix)
            except (ValueError, TypeError):
                continue
            # Old sequential barcodes have small numeric values (1, 2, 3...)
            # Scrambled barcodes map to large values — if the number is small,
            # it was generated by the old sequential scheme and needs migration
            if old_num <= len(old_barcodes):
                new_b36 = _int_to_base36(_scramble_seq(idx))
                new_barcode = f'{_BARCODE_PREFIX}{new_b36.zfill(4)}'
...
# Scramble sequential IDs into pseudo-random 4-digit base-36 codes using a
# linear congruential permutation: scrambled = (seq * A + B) mod M
# where M = 36^4 = 1,679,616 and A is coprime to M.  This is a bijection
# (every input maps to a unique output), so no collisions are possible.
_BARCODE_SPACE = 36 ** 4          # 1,679,616 possible values (0000..ZZZZ)
_BARCODE_MULTIPLIER = 1019743     # prime, coprime to _BARCODE_SPACE
_BARCODE_OFFSET = 584291          # arbitrary offset for extra scrambling
...
def _next_barcode_value(conn):
    """Generate the next barcode with a scrambled 4-digit base-36 suffix.

    Internally uses a monotonic counter (barcode_seq table) for uniqueness,
    then scrambles the value so barcodes don't reveal creation order.
    ...
    b36 = _int_to_base36(scrambled)
    return f'{_BARCODE_PREFIX}{b36.zfill(4)}'
